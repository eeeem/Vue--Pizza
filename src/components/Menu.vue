      ...
      <div class="col-sm-12 col-md-4">
        <div v-if="baskets.length>0">
          <table class="table">
            ...
            <tr>
              <td>
                <button @click="decreaseQuantity(item)" class="btn btm-sm">-</button>
                <span>{{item.quantity}}</span>
                <button @click="increaseQuantity(item)" class="btn btm-sm">+</button>
              </td>
  ...
  export default {
    ...
    computed:{
      ...
      total(){
        ...
        for (let index in this.baskets){
          ...
          tatalCost+=individuaItem.quantity*individuaItem.price
        }
        return tatalCost
      }
    },
    ...
    methods:{
      fetchData(){
        ...
        //将请求下来的数据存储到VUEX中
        this.http.get("menu.json")
          .then(res=>this.$store.commit("srtMenuItems",res.data))
      },
